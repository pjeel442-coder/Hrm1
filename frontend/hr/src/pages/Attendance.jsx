...
const Attendance = () => {
  ...
  const allCombinedRecords = useMemo(() => {
    const combined = records
      ...
      .map(r => {
        ...
        const matched = teamLiveSessions.find(s => {
          const sEmpUser = s.employeeId || s.user;
          const sEmpId = sEmpUser?._id || sEmpUser?.id || (typeof sEmpUser === 'string' ? sEmpUser : null);
          const sDateStr = typeof s.date === 'string' ? s.date.split('T')[0] : getLocalYYYYMMDD(new Date(s.date));
    ...
    teamLiveSessions.forEach(s => {
      ...
      const empRole = (empUser?.role || s.employeeRole || '').toLowerCase();
      // 🚫 HR must NEVER see Admin live sessions
      if (empRole.includes('admin')) return;
      ...
      if (dStr === todayStr) {
        ...
        if (!exists) {
          ...
          combined.push({
            ...
            user: typeof empUser === 'object' && empUser !== null ? empUser : { _id: empId, name: s.employeeName || s.userName || 'Team Member', role: empRole || 'employee' },
            date: todayStr,
    ...
    if (viewContext === 'employee') {
      ...
      if (userRole === 'hr' || window.location.pathname.startsWith('/hr')) {
        filtered = filtered.filter(r => {
          ...
          const name = (r.user?.name || r.userName || r.name || '').toLowerCase();
          const email = (r.user?.email || r.email || '').toLowerCase();
          if (role.includes('admin')) return false;
          if (name.includes('admin')) return false;
          if (name === 'dhruv mehta' || name === 'kalpesh patel') return false;
          if (email.includes('admin') || email.includes('dhruv.mehta') || email.includes('kalpesh.')) return false;
          return true;
        });
  ...
  const baseAttendanceRecords = useMemo(() => {
    ...
    return base;
  }, [allCombinedRecords, viewContext, userRole]);
