...
const Attendance = () => {
  ...
  const baseAttendanceRecords = useMemo(() => {
    let base = [...(allCombinedRecords || records || [])];
    if (viewContext === 'employee') {
      const currentUser = JSON.parse(sessionStorage.getItem('user') || '{}');
    ...
    return base;
  }, [allCombinedRecords, records, viewContext]);
  ...
  return (
    <div className="space-y-6 pb-10">
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-[#e2eae7] dark:divide-[#133029]">
              {paginatedRecords.length === 0 ? (
                <tr>
                  <td colSpan={userRole !== 'employee' ? 9 : 8} className="py-8 text-center">
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 border-r border-[#e2eae7] dark:border-[#133029]">
                      <span className="text-[11.5px] font-bold text-slate-800 dark:text-white">
                        {(() => {
                          ...
                          const isRowToday = record.date && (typeof record.date === 'string' ? record.date.split('T')[0] === getLocalYYYYMMDD(new Date()) : false);
                          const userLiveActive = isRowToday ? liveActiveSeconds : 0;
                    ...
                    <td className="px-3 py-1.5 border-r border-[#e2eae7] dark:border-[#133029]">
                      <span className="text-[11.5px] font-medium text-amber-600 dark:text-amber-400">
                        {(() => {
                          const isRowToday = record.date && (typeof record.date === 'string' ? record.date.split('T')[0] === getLocalYYYYMMDD(new Date()) : false);
                          const userLiveIdle = isRowToday ? liveIdleSeconds : 0;
      ...
      <ExportFilterModal
        ...
        defaultFilename={`attendance_my_${viewMode}`}
      />
    </div>
  );
};
